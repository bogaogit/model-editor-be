...
class WhisperMic:
    def __init__(self,model="small",device=("cuda" if torch.cuda.is_available() else "cpu"),english=False,verbose=True,energy=300,pause=2,dynamic_energy=False,save_file=True, model_root="~/.cache/whisper",mic_index=None,implementation="whisper",hallucinate_threshold=300):
    ...
    def __file_handler(self, filePath):
        try:
            with open(filePath, "rb") as audio_file:
                audio = audio_file.read()
            self.__transcribe(data=audio)
        except sr.WaitTimeoutError:
    ...
    def transcribe_file(self, filePath):
        self.logger.info("Transcribe file...")
        self.__file_handler(filePath)
        while True:
            if not self.result_queue.empty():
                return self.result_queue.get()
